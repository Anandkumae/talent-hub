...
export function ApplyForm({ user, job, setOpen }: { user: User; job: Job; setOpen: (open: boolean) => void }) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const storagePath = `resumes/${userId}/${Date.now()}-${resume.name}`;
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="grid gap-4 py-4">
      ...
      <div className="grid gap-2">
        ...
        <Input id="phone" name="phone" type="tel" defaultValue={user.phoneNumber ?? ''} placeholder="Your phone number" required />
